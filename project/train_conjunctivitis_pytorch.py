...
class CsvImageDataset(Dataset):
    ...
    def resolve_image_path(self, row: dict[str, str]) -> Path:
        ...
        raise FileNotFoundError(
            "Could not find image. Use --project-root to point at the folder "
            f"containing the dataset. Missing: {row['relative_path']}"
        )
...
def train(args: argparse.Namespace) -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_metrics["f1"] > best_f1:
            ...
            best_state = copy.deepcopy(model.state_dict())
            torch.save(best_state, output_dir / "best_conjunctivitis_efficientnet_b0.pt")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--csv", type=Path, default=Path("project/conjunctivitis_dataset.csv"))
